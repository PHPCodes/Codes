  ...
  $.fn.birthdaypicker = function( options ) {
    ...
    return this.each(function() {
      ...
      var $fieldset = $("<fieldset class='birthday-picker'></fieldset>"),
          $year = $("<select class='birth-year ll' name='birth[year]'></select>"),
          $month = $("<select class='birth-month ll' name='birth[month]'></select>"),
          $day = $("<select class='birth-day ll' name='birth[day]'></select>");
      ...
      for (var i=startYear; i>=endYear; i--) { $("<option class='dyy"+i+"'></option>").attr("value", i).text(i).appendTo($year); }
      for (var j=0; j<12; j++) { $("<option class='dmm"+(j+1)+"'></option>").attr("value", j+1).text(months[settings["monthFormat"]][j]).appendTo($month); }
      for (var k=1; k<32; k++) { $("<option class='ddd"+k+"'></option>").attr("value", k).text(k).appendTo($day); }
