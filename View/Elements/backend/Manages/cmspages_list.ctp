         ...
         <?php /*?><?php echo $this->element('adminElements/table_head'); ?><?php */?>
			<table id="sort-table"> 
				...
				<tbody> 
                    ...
                    <tr>
                        <td>
	                        <?php //echo $pages['CmsPage']['page_title']; ?>
									<?php 
										if(isset($pages))
										{										
											if(strlen($pages['CmsPage']['page_title']) >50)
											{
												$str=substr($pages['CmsPage']['page_title'],0,50);
												echo $str.'......';
											}
											else
											{
												echo $pages['CmsPage']['page_title'];
											}
										}
									?> 
                        <!--</td>preg_replace("/\s|&nbsp;/",'',$string)
                        --><td><?php echo substr(trim(strip_tags(preg_replace("/\s|&nbsp;/",' ',$pages['CmsPage']['content']))),0,80); ?></td>
                        <?php /*?><td><?php echo $pages['CmsPage']['status']; ?></td><?php */?>
                      
						
                              <?php $pageid = base64_encode(convert_uuencode($pages['CmsPage']['id'])); ?>
                          <?php  
                           if($subadmin_type==1|| @$modulepermissions['Content Management']['module_edit']==1)
              			   { 
						   ?>  
							<td> 
                            <a title="Edit" href="<?php echo HTTP_ROOT."admin/Manages/editCmsPage/".$pageid; ?>" class="btn_no_text btn ui-state-default ui-corner-all tooltip">
             ...
             <?php echo $this->element('backend/table_head20'); ?>
