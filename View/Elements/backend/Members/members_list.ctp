...
<?php
    ...
    if($member_type=='2')
    {
        $selected_module_name = 'User' ;
    }
    else if($member_type=='3')
    {
        $selected_module_name = 'Supplier' ;
    }   
?>
<div id="page-content">
			...
			<table id="sort-table"> 
				...
				{	?>				
					...
					<tr>
                        ...
                        if($member_type=='2')
                        {
                        	if($data['Member']['company_user_type'] == 'sales_person') :
                        		echo 'Sales Person'; 
                        	else : 
                        		echo 'Company User'; 
                        	endif;
                        ...
                        <?php if($member_type=='3') { ?>  
                        <td><?php echo $data['Member']['city']; ?></td>
                        <?php 
                         } ?>
                        <td><?php echo date('d F Y',strtotime($data['Member']['registered'])); ?></td>
						<?php 
						if($member_type=='3' && $data['Member']['status']=="Inactive" && $data['Member']['first_step_approval'] == "Yes" && $data['Member']['second_step_approval'] == "Yes" ) 
						{ 
						?>  
                        <td>
						<?php 
						...
						elseif(
						($member_type=='3' || $member_type=='2') && $data['Member']['status']=="Inactive"
						&&
						($data['Member']['first_step_approval']=="No" || $data['Member']['second_step_approval'] == "No") 
						)
						{
						?>
						<td>
						...
						elseif(
						$member_type=='2' && $data['Member']['status']=="Inactive"
						&&
						...
						if($member_type=='3' )
						{
							...
							if($data['Member']['first_step_approval']=='No')
							{
							?>
								<a title="1st Step Approval Pending from <?php echo date('d F Y',strtotime($data['Member']['registered'])); ?>" style="border-color:green;border-width:3px; " class="btn_no_text btn ui-state-default ui-corner-all tooltip " href="<?php echo HTTP_ROOT."admin/Members/sendpdf/".$newsid; ?>">	
							...
							?>
							<a title="1st Step Approval Completed <?php if($admin_name1 != '' || $admin_name1 != Null) { echo 'by'.' '.$admin_name1.' '."on".' '.date('d F Y',strtotime($data['Member']['date_approve_first_step_approval'])); } ?>" class="btn_no_text btn ui-state-default ui-corner-all tooltip ui-state-hover" href="javascript:void(0);">	
								<span class="ui-icon ui-icon-search"></span>
							</a>
							<?php
							}
							...
							{
								if($data['Member']['first_step_approval'] == 'Yes')
								{
							?>
								<a title="2nd Step Approval Pending from <?php echo date('d F Y',strtotime($data['Member']['registered'])); ?>" style="border-color:green;border-width:3px; " class="btn_no_text btn ui-state-default ui-corner-all tooltip send_otp" data-id="<?php echo $data['Member']['id']; ?>" rel="<?php echo $data['Member']['email']; ?>" href="javascript:void(0);">	
									<span class="ui-icon ui-icon-search"></span>
								</a>
							<?php
								}
								...
								{
							?>
								<a title="2nd Step Approval Pending from <?php echo date('d F Y',strtotime($data['Member']['registered'])); ?>" style="border-color:green;border-width:3px; " class="btn_no_text btn ui-state-default ui-corner-all tooltip"  href="javascript:void(0);">	
									<span class="ui-icon ui-icon-search"></span>
								</a>
							<?php
								}
							...
							?>
								<a title="2nd Step Approval Completed by <?php echo @$admin_name2.' '."on".' '.date('d F Y',strtotime($data['Member']['date_approve_second_step_approval'])); ?>" class="btn_no_text btn ui-state-default ui-corner-all tooltip ui-state-hover" href="javascript:void(0);">	
									<span class="ui-icon ui-icon-search"></span>
								</a>
							<?php
							} 	
							?>  
							<a title="Edit" href="<?php echo HTTP_ROOT."admin/Members/edit_member/".$newsid; ?>" class="btn_no_text btn ui-state-default ui-corner-all tooltip">
							...
							elseif($data['Member']['status']=="Inactive" && $data['Member']['first_step_approval'] == "Yes" && $data['Member']['second_step_approval'] == "Yes")
							{
							?>
								<a title="Unsuspended" class="btn_no_text btn ui-state-default ui-corner-all tooltip ui-state-hover" href="<?php echo HTTP_ROOT."admin/Members/statusMember/".$newsid; ?>">
									<span class="ui-icon ui-icon-lightbulb"></span>
								</a>
							<?php
							}
							elseif($data['Member']['status']=="Inactive" && ($data['Member']['first_step_approval'] == "No" || $data['Member']['second_step_approval'] == "No"))
							{
							?>
							<a title="Send Rejection" class="btn_no_text btn ui-state-default ui-corner-all tooltip" href="<?php echo HTTP_ROOT."admin/Members/send_rejection/".$newsid; ?>">
								<span class="ui-icon ui-icon-lightbulb"></span>
							</a>
							<?php
							}
							...
							{
							?>
								<a title="Resend Password" class="btn_no_text btn ui-state-default ui-corner-all tooltip" href="<?php echo HTTP_ROOT."admin/Members/send_password/".$newsid; ?>">	
									<span class="ui-icon ui-icon-mail-open"></span>
								</a>
							<?php
							}
							...
							<a title="Payment History" class="btn_no_text btn ui-state-default ui-corner-all tooltip" target="_blank" href="<?php echo HTTP_ROOT.'admin/Business/payment_history/'.$newsid; ?>" >
								<span class="ui-icon ui-icon-bookmark"></span>
							</a>
							<?php
										
                          
							}		 // end of member_type3 action edit functionality.
						   // end of member_type3 action.
						else
						{
							...
							?>
								...
								{
									if(
										$member_type=='2' && $data['Member']['status']=="Active"
										&&
										$data['Member']['first_step_approval']=="Yes" 
										&&
