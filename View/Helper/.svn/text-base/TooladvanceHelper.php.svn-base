...
class TooladvanceHelper extends AppHelper {
    ...
    if (!isset ($_FILES[$field_name])) return false;
     ...
     //pr($entry);die;
 		...
 		if (!is_dir($upload_path.$sub))
        $dr= mkdir($upload_path.$sub, 0777, true);
   if (!file_exists($entry['tmp_name'][$cnt])) return false;
 		      $r = copy ($entry['tmp_name'][$cnt], $upload_path . $sub . $file_name);
   return $r;
 	}
